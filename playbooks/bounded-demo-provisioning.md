...
## A Real, Common First-Time Gotcha: Missing Service-Linked Role

An AWS account that has never used a particular service before (ECS is a common example) may fail on the very first resource creation attempt with an error resembling "Unable to assume the service linked role." This is not a mistake in the setup — AWS has simply never auto-created the service-linked IAM role that service needs to operate, since it only gets created on genuine first use. Fix directly by creating the missing role explicitly, using AWS CloudShell (accessible directly from the console toolbar, running with the logged-in user's own permissions, no separate credentials needed) rather than searching for a workaround: aws iam create-service-linked-role --aws-service-name SERVICE.amazonaws.com, substituting the relevant service (ecs.amazonaws.com for ECS). This is a one-time fix per account per service — once created, the role persists for all future use of that service.
